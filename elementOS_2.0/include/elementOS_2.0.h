...
namespace eos{
    ...
    inline u_int ALLIANCE = 0; //0 = red , 1 = blue;
    ...
    inline void SystemInitialize(brain* pbrain , controller* pcontroller , competition* pcompetition){
    ...
    inline int SystemBatteryCapacity(){
        return int(eos::BRAIN->Battery.capacity()) >= 100 ? 99 : int(eos::BRAIN->Battery.capacity());
    }
    inline double SystemBatteryCurrent(){
        return eos::BRAIN->Battery.current(currentUnits::amp);
    }
    inline double SystemBatteryVoltage(){
        return eos::BRAIN->Battery.voltage(voltageUnits::volt);
    }
    ...
    inline void DrawLine(int x1 , int y1 , int x2 , int y2 , const char* color = HEX_WHITE){
        eos::LCD->setPenColor(color);
